...
select 
    ...
    TO_DATE("DATE", 'Mon DD, YY') + INTERVAL '2000 years' AS transaction_date, -- Convert transaction date to a date format
    "TRANSACTION DETAILS" AS transaction_details,
    CAST("CHQ.NO." AS INT) AS cheque_number, -- Convert data type into INT
    TO_DATE("VALUE DATE", 'Mon DD, YY') + INTERVAL '2000 years' AS value_date, -- Convert value date to a date format
